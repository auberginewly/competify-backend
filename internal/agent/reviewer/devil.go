...
// marketingHypeWords are used by the reverse-evidence strategy as a deterministic
// stand-in for real VikingDB Grep (Phase 6). If the payload contains these words,
// DevilsAdvocate treats the claim as unverified marketing language.
...
//  1. Reverse-evidence search (deterministic rules until VikingDB Phase 6).
...
// NewDevilsAdvocate creates a DevilsAdvocate with an optional audit chain.
func NewDevilsAdvocate(auditChain ...*provenance.AuditChain) *DevilsAdvocate {
	ba := agent.BaseAgent{Role: "devils_advocate"}
	if len(auditChain) > 0 {
		ba.AuditChain = auditChain[0]
	}
	return &DevilsAdvocate{BaseAgent: ba, MaxRounds: 3}
}
...
func (d *DevilsAdvocate) challenge(_ context.Context, result *schema.AnalysisResult) []schema.Conflict {
	var conflicts []schema.Conflict

	// Strategy 1: reverse-evidence (deterministic marketing-hype detection).
	if hype := findHypeWords(result.Payload); len(hype) > 0 {
